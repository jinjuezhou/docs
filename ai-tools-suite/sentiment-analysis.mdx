...
| language          | String   |         | When type=2, the language of the video must needs to be provided。 Supplying the input language in ISO-639-1 format                              |
...
| data          | Object   | `{ status:1, _id:"", result:"" }` | video_status: the status of video：【1:queueing, 2:processing, 3:completed, 4:failed】 result:  sentiment analysis result【"POSITIVE", "NEGATIVE"】 _id: Interface returns data |
