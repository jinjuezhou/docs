...
OpenAPI uses API keys for authentication. Get your API token from our API interfaces . We provide open APIs for main GenAI Marketing platform by clicking on the top left API button on this page [Content](https://akool.com/tools) and face swap API button on this page [Faceswap](https://faceswap.akool.com/).
...
<Note>Please note that the token generated each time is only valid for 5 minutes. It is recommended that you generate a new token for each request.</Note>
